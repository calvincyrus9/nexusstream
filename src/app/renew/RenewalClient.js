...
const RenewalClient = () => {
  ...
  const initialDuration = searchParams.get('duration') || '1 Month';
  const initialDevices = parseInt(searchParams.get('devices'), 10) || 1;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitStatus({ success: true, message: 'Thank you for your renewal request. We will contact you soon with the next steps.' });
      e.target.reset();
      setDuration(searchParams.get('duration') || '1 Month');
      setDevices(parseInt(searchParams.get('devices'), 10) || 1);
    } catch (error) {
  ...
  return (
    <motion.div 
      className="bg-slate-800/50 p-8 rounded-2xl border border-slate-700 w-full"
