...
const PricingCard = ({ duration, price, popular, devices, months }) => {
  ...
  // Calculate total price for multi-month plans
  const totalPrice = months > 1 ? price * months : price;
  ...
  return (
    <div className={`relative rounded-2xl overflow-hidden bg-slate-800/50 backdrop-blur-sm ${popular ? 'border-2 border-blue-500' : 'border border-slate-700'}`}>
      ...
      <div className={`p-8 ${popular ? 'pt-16' : ''}`}>
        ...
        <div className="mb-6">
          <span className="text-4xl font-bold text-white">${totalPrice.toFixed(2)}</span>
          <span className="text-slate-400">
            {months > 1 || devices > 1
              ? ` total for ${devices} device${devices > 1 ? 's' : ''}`
              : ` / month`}
          </span>
        </div>
        
        <ul className="space-y-3 mb-8 text-slate-300">
          {[`${devices} Connected Device${devices > 1 ? 's' : ''}`, 'All Channels Access', '4K Streaming', '7-Day Catch Up', '24/7 Support'].map((item, index) => (
          ...
          {plans.map(plan => {
            ...
            return (
              <PricingCard 
                ...
                months={months} // pass months to card
              />
            )
          })}
        </div>
