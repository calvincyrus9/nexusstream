...
const PricingCard = ({ duration, price, popular, devices }) => {
  ...
  // Extra feature for 3, 6, 12 month plans
  if (["3 Months", "6 Months", "12 Months"].includes(duration)) {
    features.push("IPTV Player Included");
  }
      ...
      <div className={`p-8 ${popular ? 'pt-16' : ''}`}>
        ...
        <div className="mb-6">
          ...
          <span className="text-slate-400">
            {devices > 1 ? ` total for ${devices} devices` : ` / ${duration} `}
          </span>
        </div>
...
const Pricing = () => {
  ...
  const plans = [
    ...
    { duration: "12 Months", basePrice: 91.99, popular: false },
  ];
